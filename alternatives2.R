alternatives2 = function (dat, g1, group1 = "Species", group2 = "",
  ...
  fieldsmissing(dat, fields = c("ID", "x", "y", 
                                "x_original", "y_original", "Correction", 
                                "Exclude"))
  cn <- names(dat)
  f3 <- match(group1, cn)
  ...
  while (sum(sel) < n) {
    ...
    if (nchar(group2) > 0) {
      ...
      if (ng2 > 0) {
        ...
        legend("bottomleft", legend = as.character(g2), 
               col = cols, pch = 18)
      }
    }
    ...
    if (f1 == 7) {
      ...
      dat$Modified[ffa] <- mod
      xyp <- paste("new x = ", dat$x[ffa], "; y = ", 
                   dat$y[ffa], "; Locality = ", dat$LocalityName[ffa], 
                   sep = "")
      print(xyp)
    }
    sel[fs] <- TRUE
